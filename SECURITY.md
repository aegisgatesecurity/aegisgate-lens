...
## Key rotation and offline storage

The Ed25519 signing keypair for the toxicity bundle (
and the corresponding private key) is held **OFFLINE** — the private key is
NOT in this repository. This is by design:

- The v0.2.2 tag was signed with a private key that is held offline in `lens-repo-bootstrap-v02/.v0.3-keys-offline/` (gitignored, local only)- The v0.3.0-rc1 release (and beyond) sign bundles with a NEW keypair held
  offline, similar to the Linux kernel signing key model
- Only the PUBLIC key (`keys/lens-toxicity-pub.pem`) is in the public repo;
  the private key is never committed

If you discover the private key has been leaked (e.g., found in a public
repo, pastebin, etc.), email **security@aegisgatesecurity.io** immediately
so we can rotate. The legacy v0.2.2 bundles will need to be re-signed and
re-released.

See `docs/KEY_ROTATION_POLICY.md` (v0.3.0 addendum) for the full policy.
