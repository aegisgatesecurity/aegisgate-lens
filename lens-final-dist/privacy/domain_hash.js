/* SPDX-License-Identifier: Apache-2.0
   ...
   audited by the browser vendor, and counts as platform stdlib —
...
(function () {
  ...
  /** Required length of the truncated hash. Must match domain_hash.go. */
  const DOMAIN_HASH_LENGTH = 16;

  /** Algorithm. SHA-256 only; no SHA-1, no MD5, no SHA-3 in v0.1. */
  ...
  /**
   * Compute the 16-character lowercase hex SHA-256 prefix of the
   * given hostname. Async because crypto.subtle.digest is async.
   *
   * @param {string} hostname Lowercased before hashing; pass it
   *   user-typed (e.g., "Chat.OpenAI.com") and we'll lowercase.
   * @returns {Promise<string>} A 16-character lowercase hex string.
   */
  ...
  /**
   * Synchronous SHA-256 of a byte array. Hand-rolled; test path only.
   *
   * @param {Uint8Array} bytes
  ...
  /**
   * Synchronous variant of computeDomainHash. Used by tests; not
   * production. The browser-side runtime path is the async
   * crypto.subtle.digest above.
   *
  ...
  /** @type {ReadonlyArray<{hostname: string, hash: string}>} */
  ...
  NS.privacy.domainHash = Object.freeze({
    DOMAIN_HASH_LENGTH,
    computeDomainHash,
    computeDomainHashSync,
    KNOWN_VECTORS,
  });
})();